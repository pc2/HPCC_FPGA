        ...
        for (uint row = 0; row < BLOCK_SIZE; row++) {
            ...
            for (uint col = 0; col < BLOCK_SIZE / CHANNEL_WIDTH; col++) {
                ...
                ulong ls_address_row = block_row * BLOCK_SIZE * BLOCK_SIZE * width_in_blocks +
                                        block_col * BLOCK_SIZE + row * BLOCK_SIZE * width_in_blocks;
                ...
                switch (tblock % 3) {
                    // Read in a block of A and transpose it halfway
                    case 0:
                        ...
                        break;
                    // Transpose block of A by reading it from local memory and add block of B
                    case 1:
                        __attribute__((opencl_unroll_hint(CHANNEL_WIDTH)))
                        for (unsigned unroll_count = 0; unroll_count < CHANNEL_WIDTH; unroll_count++) {
                            unsigned rot_in = ((CHANNEL_WIDTH + unroll_count - row) * (BLOCK_SIZE / CHANNEL_WIDTH)) &
                                                                                                        (BLOCK_SIZE - 1);
                            unsigned row_rotate = base + offset + rot_in;
                        ...
                        __attribute__((opencl_unroll_hint(CHANNEL_WIDTH)))
                        for (unsigned unroll_count = 0; unroll_count < CHANNEL_WIDTH; unroll_count++) {
                            data_chunk[unroll_count] = rotate_out[(unroll_count + rot) & (CHANNEL_WIDTH - 1)];
                        }
